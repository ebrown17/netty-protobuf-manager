...
public class ClientConnector {
	...
	private static final long MAX_RETRY_TIME = 30L; // 1 min
	private static final int maxRetriesBeforeIncrease = 30; // 5 mins until connection interval is changed - 6 tries a minute 6/30 = 5
	...
	private long calculateRetryTime(){
		...
		if(retryCount >= maxRetriesBeforeIncrease){
			logger.debug("calculateRetryTime {}>={} setting {} as retry interval: total time retrying {} seconds",
					retryCount,maxRetriesBeforeIncrease,MAX_RETRY_TIME,((retryCount-maxRetriesBeforeIncrease)*MAX_RETRY_TIME) + (maxRetriesBeforeIncrease*RETRY_TIME) );
			retryCount++;
			return MAX_RETRY_TIME;
		}
		else {
			logger.debug("calculateRetryTime {}<{} setting {} seconds as retry interval: total time retrying {} seconds",retryCount,maxRetriesBeforeIncrease,RETRY_TIME,RETRY_TIME*retryCount);
			retryCount++;
			return RETRY_TIME;
		}
	}
